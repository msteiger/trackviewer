...
{
	private static final File folder = new File(System.getProperty("user.home") + File.separator + "trackviewer");
...
//		Closest index in other track: 968 -- 986
	...
	private static void asdf(List<Range<Integer>> gaps1, Track track1, Track track2)
	{
		...
		{
			double gapDist = pts1.get(range.getEnd()).getDistance() - pts1.get(range.getStart()).getDistance();
			long gapTime = pts2.get(range.getEnd()).getTime().getTime() - pts2.get(range.getStart()).getTime().getTime();
			...
			double overlapDist = pts2.get(otherEndIdx).getDistance() - pts2.get(otherStartIdx).getDistance();
			long overlapTime = pts2.get(otherEndIdx).getTime().getTime() - pts2.get(otherStartIdx).getTime().getTime();
